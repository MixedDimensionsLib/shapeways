...
var oauth = require("mixeddimensions-oauth");
 ...
 * @param {client~resultsCallback} [callback] - function to call when finished
 **/
 client.prototype.post = function(url, body, header, callback){
     ...
     this.connection.post(url, this.oauthToken, this.oauthSecret, body, header, function(error, data, result){
         if(error){
             return callback(error);
         }
 ...
 * The `file` parameter is base64 + URI encoded before sending to server
 *
 * @method partner order
 * @memberof client
 * @instance client
 *
 * @param {object} params - the order data
 * @param {client~resultsCallback} [callback] - function to call when finished
 **/
client.prototype.order = function(params, callback){
    var required = ["firstName", "lastName", "country", "zipCode","state", "city", "address1", "paymentVerificationId", "paymentMethod", "shippingOption"];
    var missing = [];
    required.forEach(function(property){
        if(!(property in params)){
            missing.push(property);
    ...
    this.post(this.url("/orders/"), JSON.stringify(params), "application/json", callback);
};
